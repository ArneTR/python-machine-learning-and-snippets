...
# train_test_split -> If needed do BEFORE Scaling
## Important: Scaling must be done AFTER train_test_split! Otherwise the means / std.dev are not correct anymore
...
s = MinMaxScaler() # Use if you have strict boundaries but want a linear scaling only
s = FunctionTransformer(np.log1p, validate=True) # Use this if the data is highly skewed or very condensed in some parts of it's range. LogTransform will stretch condensed parts and 
