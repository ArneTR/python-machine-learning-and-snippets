...
# beware however, that this will make interpreation in causal scenarios
# VERY problematic. Run it once with and once without and hope that
# the values do not differ too much. Use the non-standardized version
# if possible in causal scenarios. For pure prediction it is no problem to standardize.
#
# Alternative: "Alternatively, base it on the correlation matrix, since correlations are themselves standardized.  This is generally an option in your software, and is likely the default.  Just make sure."
# => How I do that in Python?! TODO!
######################################
X = df.copy()
y = X.pop('price')
X = X.loc[:, features]
...
## A typical cutoff point is seldomly done here, but rather by looking at the eigenvalues
...
## Although this view is very similar to explained_variance_ratio_ it helps because usually a value less than 50% is a good cutoff value to drop a component as feature and RATHER
## use an original component as feature and then have a bit more of overlap in the variances.
##
## In SPSS by default every column is dropped that has less than 1 original variable ... this is nowadys seen as too strict.
...
##    PC1 consists of a mix of all. Specifically in the contrast that low values of  highway_mpg (miles per gallon) come together with high values of engine_size,horsepower, curb_weight and vice-versa
##      a good name would be luxury cars (low mpg and high horsepower) vs economy/mini cars (high mpg, but low horespower)
...
## Seletion of columns by loadings
##
## In the steps above we discussed selecting columns based on the variance they explain. It is very easy in this code example, because actually only keeping
## PC1 is a very viable solution.
## However, in other scenarios it is not so clear and then we run into the fact, that the loadings should be focused in one component. WE see this problem
## in the prior example, where PC1 and PC2 load heavy on highway_mpg and engine_size. Typically, if we keep more than one PC, we want the high loadings to be exclusive there.
## This is a very important goal!
## To achieve this we would use and try other rotation algorithms and see if they deliver better more exclusive results.
## This transforms the result very strongly and components are even less interpretable afterwards.
## Also this cannot be done in Python at the moment and must be done in R: https://stackoverflow.com/questions/24300853/principal-component-analysis-with-equamax-rotation-in-r
## The other option is to not include the feature in the PCA and rather use it
## separately. This increases the dimensionality of the resulting feature-space
## but makes PCs interpretable and useable.


## Optimizations
##
## - Try removing features that are highly correlated. They can confuse the PCA algorithm
## - If you cannot make sense of a feature through the interpretation of the
##     loadings than maybe remove the feature from the PCA and include it
#      separately without PCA applied to it.
