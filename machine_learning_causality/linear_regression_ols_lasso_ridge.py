...
## Lasso and ridge should generally not be used when transformations are
## applied, cause the transformed data is mostly uninterpretable
...
## Assummptions
##
## These are typically only done in a causality approach


## Assumptions. When doing statistics and not machine-learning-prediction
## we must check these!
#
# https://en.wikipedia.org/wiki/Ordinary_least_squares#Assumptions
# 
# - Exogenity
#    - If exongenity does not hold Instrumented Variables may help (See Wikipedia)
# - Covariates must be linear independent. No Multicolinearity
    # Model becomes instable and unusable, or cannot be learned even (fails)
# - No autocorrelation
    # This may fail in time-series data or paired measurements or if observations are not Independent (i.i.d.). 
    # Maybe generalized-least-squares can help? (Wikipedia)
# - The residuals must be normally distributed
    # NOT needed strictly. But when it is, then OLS is the best possible way to make a linear model (Another non-linear model might still be better! But GLS or any other Least-Squares will not). 
# - TODO: More! There are 4. homoskedacity etc.
